...
type DockerNameToUUIDMap struct{}
type DockerNameToUIDMap struct{}

func (d *DockerNameToUUIDMap) MapContainerName(container_name string) (string, error) {
...
func (d *DockerNameToUIDMap) MapContainerName(container_name string) (string, error) {
	endpoint := "unix:///var/run/docker.sock"
	client, err := docker.NewClient(endpoint)
	if err != nil {
		fmt.Println("Unable to connect to docker server:", err.Error())
